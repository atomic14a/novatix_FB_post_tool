...
async function publishToFacebookGraphApi(
  ...
  try {
    ...
    if (isLink && !isActuallyLocalhost) {
      ...
      const linkToPost = `${appUrl}/api/card/${postId}`;
    ...
    return { success: false, error: err instanceof Error ? err.message : "Failed to reach Facebook API" };
  }
}
...
export async function createPost(formData: {
  ...
  if (formData.status === "published") {
    ...
    const fbRes = await publishToFacebookGraphApi(newPost.id, pageData.page_id, pageData.page_access_token, {
      title: formData.title,
...
export async function updatePost(
  ...
  const postPayload: Record<string, unknown> = { ...formData, updated_at: new Date().toISOString() };
  ...
  if (formData.status === "published") {
    ...
    const fbRes = await publishToFacebookGraphApi(id, pageData.page_id, pageData.page_access_token, fbPayload);
